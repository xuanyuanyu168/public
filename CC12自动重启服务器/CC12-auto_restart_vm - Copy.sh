...
if [[ -z ${_SERVERLIST_NAME_} ]]; then
	if [[ -z ${_HOSTNAME_} ]]; then
		...
		export _RUN_MODE_=single
	fi
else
	if [[ -z ${_HOSTNAME_} ]]; then
		export _RUN_MODE_=file
...
export _WORKSPACE_DIR_PATH_=`${PWD}`
...
if [[ ${_HOSTNAME_} =~ -hdb ]];then
	echo -e "\033[31mThe ${_HOSTNAME_} is a database server,Please don't restart!!!\033[0m"
    echo "The ${_HOSTNAME_} is a database server,Please don't restart!!!"
	echo ${_HOSTNAME_} >> ${_HDB_SERVER_LIST}
	`${_EXIT_}`
fi
...
dc=`echo ${_HOSTNAME_} | awk -F'-' '{ printf $1}'`
...
result=`timeout 5 telnet ${1} ${2} 2>/dev/null | grep Escape | wc -l`
...
CHECK_SERVER_CONNECT_STATUS () {

echo "========================================================="
echo "=====   ${_HOSTNAME_}  restart  starting  ====="
echo "========================================================="
		
# CHECK_SERVER_CONNECT_STATUS ${_HOSTNAME_} $password
FAIL_COUNT=0
for ((i=1;i<=3;i++));do
	if `OS_COMMAND ${1} ${password} "/usr/bin/hostname" > /dev/null`;then
#	if `OS_COMMAND ${_HOSTNAME_} $password "/usr/bin/hostaname" > /dev/null`;then
		echo -e "\033[32m${1} is alive, don't restart\033[0m"
		exit 0
	else
		sleep 10
		let FAIL_COUNT++
...
if [ ${FAIL_COUNT} -eq 3 ];then
	echo ${current_time}  ${1} >> ${_RESTART_SERVER_LIST}
fi
}
...
if [[ ${_RUN_MODE_} == "single" ]]; then
	_EXIT_="exit 1"
	DISABLE_HANA_SERVER ${_HOSTNAME_}
	CHECK_VM ${_HOSTNAME_}
	CHECK_SERVER_CONNECT_STATUS ${_HOSTNAME_}
else 
	if [ -s ${_SERVERLIST_NAME_} ];then
		_EXIT_="continue"
		for _HOSTNAME_ in `cat ${_SERVERLIST_NAME_}`;do
			DISABLE_HANA_SERVER ${_HOSTNAME_}
			CHECK_VM ${_HOSTNAME_}
			CHECK_SERVER_CONNECT_STATUS ${_HOSTNAME_}
		done
	else
		echo "The ${_SERVERLIST_NAME_} file is empty and there is no server to check."
	fi
fi

if [ -s ${_RESTART_SERVER_LIST} ];then
	for _HOSTNAME_ in `cat ${_RESTART_SERVER_LIST}`;do
	...
	echo "No server needs to be restarted"
fi
